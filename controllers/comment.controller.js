 ...
 export const createComment=async(req,res,next)=>{
    try
    {
        ...
        const comment=await Comment.create({video:video._id,user:req.user._id,text:text.trim()});
        const populateComment=await Comment.findById(comment._id).populate("user","username avatar");
        res.status(201).json({success:true,message:"comment addded",comment:populateComment});
    }
    catch(error)
    {
        next(error);
 ...
 export const   updateComment=async(req,res,next)=>{
    try
    {
        ...
        {
            return res.status(403).json({success:false,message:"you can edit your own comment"});
        }
        const {text}=req.body;
        if(!text || !text.trim())
        {
